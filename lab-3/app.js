(function () {
  ...
  // [ADD] Mobile swipe needs the wrapper element to attach touch events.
  // (kept separate so original declaration line stays intact)
  var boardWrap; // <-- reference to ".board-wrap" (added)
  ...
  // [ADD] Mobile swipe state
  // Storing initial touch point and using small pixel threshold
  var touchStartX = 0;
  var touchStartY = 0;
  ...
  // Recording start coordinates on touchstart and deciding the direction on touchend
  function onTouchStart(e) {
    if (isGameOver || leaderOverlay.classList.contains("show")) return;
    if (e.touches && e.touches.length === 1) {
      touchStartX = e.touches[0].clientX;
      touchStartY = e.touches[0].clientY;
    }
    // preventing browser gestures/scroll to avoid stealing the swipe
    e.preventDefault();
  }
  ...
  function wireControls() {
    ...
    // [ADD] Bind touch events to the board wrapper
    // Use passive:false so preventDefault() works and stops page scroll
    if (boardWrap) {
  ...
  function cacheDom() {
    ...
    // [ADD] cache the board wrapper element for touch events
